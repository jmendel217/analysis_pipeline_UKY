...
if [[ "$con" == "human" ]] ; then
   for i in $dir/*.1.fastq ; do 
      ...
      # Launch job
      sbatch --export="$OPTS" -J "Trim-$b" --account=$QUEUE --partition=$QOS --error "$dir"/"Trim-$b"-%j.err -o "$dir"/"Trim-$b"-%j.out  $pac/run_human.pbs | grep .;
   done ;
elif [[ "$con" == "fast" ]] ; then
   for i in $dir/*.1.fastq ; do 
      b=$(basename $i .1.fastq) ;
      ...
      # Launch job
      sbatch --export="$OPTS" -J "Trim-$b" --account=$QUEUE --partition=$QOS --error "$dir"/"Trim-$b"-%j.err -o "$dir"/"Trim-$b"-%j.out  $pac/run_fast.pbs | grep .;
   done ;
else
   echo "Error in filtration parameter, aborting..." >&2
   exit 1
fi ;
